#!/usr/bin/env python3
"""Validate key constraints in a minimind-knowledge-explainer SKILL.md."""
...
def validate_rules(text: str) -> list[str]:
    rules = [
        ...
        Rule("缺少外部依据联动：涉及最新事实时必须调用 `$minimind-web-search`。", needles=("$minimind-web-search",)),
    ]
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="检查 minimind-knowledge-explainer SKILL.md 是否保留关键质量约束。"
    )
    parser.add_argument("skill_path", help="目标 minimind-knowledge-explainer/SKILL.md 路径")
    ...
    if missing:
        print("失败：minimind-knowledge-explainer 缺少以下关键质量约束：")
        for item in missing:
            print(f"- {item}")
        return 1

    print("通过：minimind-knowledge-explainer 关键质量约束检查通过。")
    return 0
